...
//resolucion puntos 1 y 2 de otra forma simplificada (puto):
...
function getBonifxCofre($cofre)
{
    ...
    foreach ($cofre as $moneda) {
        if (isset($valores[$moneda])) {
            $total += $valores[$moneda];
        }
    }

    return $total;
}
$cofre = ['oro', 'plata', 'cobre', 'oro', 'bronce', 'oro', 'bronce'];
echo getBonifxCofre($cofre); 
...
$cofre = ['oro', 'plata', 'cobre', 'oro', 'bronce'];
$resultado = getBonifxCofre($cofre);

echo "Total: $" . $resultado['total'] . "\n";
echo "Detalle de monedas:\n";
foreach ($resultado['monedas'] as $tipo => $cantidad) {
